...
import { useCanvasSnapshot } from '@/lib/useCanvasSnapshot';
...
export default function ConfiguratorContent() {
  ...
  const { saveBuild, isStorageFull, MAX_BUILDS } = useBuilds();
  const { canvasRef, captureSnapshot } = useCanvasSnapshot();
  ...
  const handleSave = () => {
    if (isStorageFull()) {
      showToast(`Gallery is full! Maximum ${MAX_BUILDS} builds allowed.`);
      return;
    }
    
    const thumbnail = captureSnapshot();
    saveBuild(config, thumbnail || undefined);
